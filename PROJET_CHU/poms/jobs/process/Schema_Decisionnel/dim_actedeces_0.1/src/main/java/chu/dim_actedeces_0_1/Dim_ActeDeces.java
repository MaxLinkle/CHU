...
/**
 * Job: Dim_ActeDeces Purpose: <br>
 * Description:  <br>
 * @author 
 * @version 7.3.1.20200219_1130
 * @status 
 */
public class Dim_ActeDeces implements TalendJob {
...
public String getHDFS_Cluster_NameNodeUri(){
	...
	private final String jobName = "Dim_ActeDeces";
	...
	public void printStackTrace() {
		...
		if (!(e instanceof TDieException)) {
			if(e instanceof TalendException){
				...
				Dim_ActeDeces.this.exception = e;
			}
		}
		if (!(e instanceof TalendException)) {
		try {
			for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
				if (m.getName().compareTo(currentComponent + "_error") == 0) {
					m.invoke(Dim_ActeDeces.this, new Object[] { e , currentComponent, globalMap});
					break;
				}
			}
...
public static class row2Struct implements routines.system.IPersistableRow<row2Struct> {
    final static byte[] commonByteArrayLock_CHU_Dim_ActeDeces = new byte[0];
    static byte[] commonByteArray_CHU_Dim_ActeDeces = new byte[0];
	...
	private String readString(ObjectInputStream dis) throws IOException{
		...
		if (length == -1) {
			...
			if(length > commonByteArray_CHU_Dim_ActeDeces.length) {
				if(length < 1024 && commonByteArray_CHU_Dim_ActeDeces.length == 0) {
   					commonByteArray_CHU_Dim_ActeDeces = new byte[1024];
				} else {
   					commonByteArray_CHU_Dim_ActeDeces = new byte[2 * length];
   				}
			}
			dis.readFully(commonByteArray_CHU_Dim_ActeDeces, 0, length);
			strReturn = new String(commonByteArray_CHU_Dim_ActeDeces, 0, length, utf8Charset);
		}
		return strReturn;
	}
    ...
    public void readData(ObjectInputStream dis) {
		...
		synchronized(commonByteArrayLock_CHU_Dim_ActeDeces) {
...
org.apache.hadoop.fs.FileSystem fs_tHDFSOutput_1 = null;
	...
	org.apache.hadoop.fs.Path path_tHDFSOutput_1 = new org.apache.hadoop.fs.Path("/user/cloudera/CHU/Dim_ActeDeces/Dim_ActeDeces.txt");
 ...
 */
	...
	currentComponent="tUniqRow_1";
					...
					if(execStat) {
						runStat.updateStatOnConnection(resourceMap,iterateId,0,0,"deces");
					}
			...
			public int hashCode() {
				if (this.hashCodeDirty) {
					...
					int result = DEFAULT_HASHCODE;
								...
								result = prime * result + ((this.Numero_d_acte_du_deces == null) ? 0 : this.Numero_d_acte_du_deces.hashCode());
...
// # Output table : 'deces'
deces_tmp.Numero_d_acte_du_deces = row1.Numero_d_acte_du_deces ;
deces = deces_tmp;
// ###############################
...
row2 = null;			
if(deces.Numero_d_acte_du_deces == null){
	finder_tUniqRow_1.Numero_d_acte_du_deces = null;
}else{
	finder_tUniqRow_1.Numero_d_acte_du_deces = deces.Numero_d_acte_du_deces.toLowerCase();
}	
...
if(deces.Numero_d_acte_du_deces == null){
	new_tUniqRow_1.Numero_d_acte_du_deces = null;
}else{
	new_tUniqRow_1.Numero_d_acte_du_deces = deces.Numero_d_acte_du_deces.toLowerCase();
}
		
		keystUniqRow_1.add(new_tUniqRow_1);if(row2 == null){ 
	
	row2 = new row2Struct();
}row2.Numero_d_acte_du_deces = deces.Numero_d_acte_du_deces;					
...
globalMap.put("tUniqRow_1_NB_DUPLICATES",nb_duplicates_tUniqRow_1);
				...
				if(execStat){
			  		runStat.updateStat(resourceMap,iterateId,2,0,"deces");
			  	}
 ...
 */
    ...
    public static void main(String[] args){
        final Dim_ActeDeces Dim_ActeDecesClass = new Dim_ActeDeces();

        int exitCode = Dim_ActeDecesClass.runJobInTOS(args);
    ...
    public int runJobInTOS(String[] args) {
        ...
        try {
            ...
            java.io.InputStream inContext = Dim_ActeDeces.class.getClassLoader().getResourceAsStream("chu/dim_actedeces_0_1/contexts/" + contextStr + ".properties");
            if (inContext == null) {
                inContext = Dim_ActeDeces.class.getClassLoader().getResourceAsStream("config/contexts/" + contextStr + ".properties");
            }
            if (inContext != null) {
...
this.globalResumeTicket = true;//to run tPostJob
        ...
        if (false) {
            System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : Dim_ActeDeces");
        }
...
/************************************************************************************************
 *     87329 characters generated by Talend Open Studio for Big Data 
